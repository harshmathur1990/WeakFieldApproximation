...
wave = np.linspace(853.9444, 854.7444, 1001)
...
atlas = np.loadtxt('catalog_8542.txt')

ind = list()


for w in vac_to_air(wave):
    ind.append(np.argmin(np.abs(atlas[:, 0] - w * 10)))

ind = np.array(ind)

plt.plot(wave, atlas[ind, 1] / atlas[ind, 1][0], label='FTS')
plt.plot(wave, IwaveRef / IwaveRef[0], label='Reference FAL')
plt.plot(wave, IwaveCons / IwaveCons[0], label='Reference Cons')
plt.plot(wave, IwaveLte / IwaveLte[0], label='Reference LTE n_e')
plt.legend()
plt.show()
